...
def query_vector_db(query_text):
    ...
    cursor.execute("""
        SELECT id, book_id, chapter_id, heading_id, chunk_index,
               book_title, chapter_title, content,
               1 - (embedding <=> %s::vector) AS similarity
        FROM maududi_documents
...
def generate_response(query, context, formatting, model=DEFAULT_MODEL):
    ...
    context_text = "\n\n---\n\n".join([
    f'Excerpt {i+1} from the book "{item["book_title"]}", Chapter: "{item["chapter_title"]}", Chunk: {item["chunk_index"]}:\n\n"{item["content"]}"'
    for i, item in enumerate(context)
    ])

    # Create GPT prompt
    prompt = f"""
    You are an AI assistant. Answer the question based on the following excerpts from the writings of Maulana Maududi. These excerpts are taken from his original Urdu books and digitized for the Read Maududi App.
    ...
    - Include numbered references for the excerpts that support your answer, at the end in the following format:
    - Excerpt [[[1]]]: [Book Title], Chapter: [Chapter Title], Chunk: 2
